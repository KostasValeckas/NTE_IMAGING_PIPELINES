...
class ReductionPipeline:
    ...
    def create_setup_table(self):
        ...
        for file in self.science_files:
            ...
            window = (
                self.instrument.get_header_value(
                    hdul, self.instrument.detector.window_keyword
                )
                or "UNKNOWN"
            )
            bin_x = (
                self.instrument.get_header_value(
                    hdul, self.instrument.detector.bin_x_keyword
                )
                or "UNKNOWN"
            )
            bin_y = (
                self.instrument.get_header_value(
                    hdul, self.instrument.detector.bin_y_keyword
                )
                or "UNKNOWN"
            )

            filter_names = self.instrument.get_header_values(
                hdul, self.instrument.filter_keyword
            ) or ["UNKNOWN"]
            ...
            self.logger.debug(
                f"File: {file}, Window: {window}, Bin X: {bin_x}, Bin Y: {bin_y}, Filter: {filter_name}"
            )
    ...
    def get_fits_data(self, filepath):
        ...
        extension = (
            self.instrument.data_hdu_extension
            if self.instrument.data_hdu_extension is not None
            else 0
        )
    ...
    def determine_bias_configurations(self):
        ...
        if not getattr(self, "setup_table", None):
            self.logger.info(
                "No setup table available to determine bias configurations."
            )
            self.bias_configurations = {}
            self.science_to_bias_map = {}
        ...
        bias_lookup = {
            (cfg["window"], cfg["bin_x"], cfg["bin_y"]): bidx
            for bidx, cfg in self.bias_configurations.items()
        }
        ...
        next_idx = (
            max(self.bias_configurations.keys()) + 1 if self.bias_configurations else 0
        )
        ...
        for bf in bias_files:
            ...
            try:
                ...
                try:
                    ...
                    self.logger.warning(
                        f"Failed to read window keyword using get_header_value for bias file {bf}: {e}"
                    )
                    w = "UNKNOWN"
                ...
                try:
                    ...
                    self.logger.warning(
                        f"Failed to read bin_x keyword using get_header_value for bias file {bf}: {e}"
                    )
                    bx = "UNKNOWN"
                ...
                try:
                    ...
                    self.logger.warning(
                        f"Failed to read bin_y keyword using get_header_value for bias file {bf}: {e}"
                    )
                    by = "UNKNOWN"

            except Exception as e:
                self.logger.warning(
                    f"Failed to read header keywords from bias file {bf}: {e}"
                )
            ...
            if matched_idx is None:
                ...
                self.bias_configurations[matched_idx] = {
                    "window": w,
                    "bin_x": bx,
                    "bin_y": by,
                    "files": [bf],
                ...
                next_idx += 1
                self.logger.info(
                    f"Created new bias configuration {matched_idx} for key {key} and added file {bf}"
                )
            else:
                files_list = self.bias_configurations[matched_idx].setdefault(
                    "files", []
                )
                if bf not in files_list:
                    files_list.append(bf)
                    self.logger.info(
                        f"Appended bias file {bf} to bias configuration {matched_idx}"
                    )
        ...
        self.logger.info(
            f"Found {len(self.bias_configurations)} unique bias configurations"
        )
        ...
        try:
            ...
            serializable_map = {
                str(k): (v if v is not None else None)
                for k, v in self.science_to_bias_map.items()
            }
    ...
    def make_master_bias(self):
        ...
        for key, configuration in self.bias_configurations.items():
            ...
            for bias_file in configuration.get("files", []):
                ...
                if data is None:
                    self.logger.warning(
                        f"Could not read bias file {bias_file}, skipping"
                    )
                    continue
            ...
            if not np.any(finite_mask):
                self.logger.warning(
                    "No finite pixels in combined median; skipping masking and histogram"
                )
                ...
                self.logger.info(
                    f"Master bias initial median: {median_val:.3f}, ±0.1*median = [{lower_small:.3f}, {upper_small:.3f}]"
                )
                ...
                if kept_vals_small.size == 0:
                    ...
                    self.logger.warning(
                        "No pixels remain after ±0.1*median masking on copy; using all finite pixels to estimate sigma"
                    )
                ...
                try:
                    sigma_est = (
                        float(np.std(kept_vals_small, ddof=1))
                        if kept_vals_small.size > 1
                        else float(np.std(kept_vals_small, ddof=0))
                    )
                except Exception:
                ...
                if sigma_est == 0.0 or not np.isfinite(sigma_est):
                    ...
                    self.logger.warning(
                        f"Estimated sigma is zero or non-finite; using fallback sigma={sigma_est:.6g}"
                    )

                self.logger.info(
                    f"Estimated sigma from ±0.1*median-masked copy: {sigma_est:.3f}"
                )
                ...
                self.logger.info(
                    f"Masking original combined median with median ± 5*sigma = [{lower_5s:.3f}, {upper_5s:.3f}]"
                )
                ...
                n_total = data.size
                self.logger.info(
                    f"Masked {n_masked}/{n_total} pixels outside median ± 3*sigma or non-finite"
                )
                ...
                if kept_vals_small.size == 0:
                    self.logger.warning(
                        "No pixels available to plot histogram after ±0.1*median masking"
                    )
                else:
                    # subsample for plotting if very large
                    if kept_vals_small.size > 200_000:
                        idx = np.random.choice(
                            kept_vals_small.size, size=200_000, replace=False
                        )
                    ...
                    plt.axvline(
                        ...
                        linestyle="--",
                        linewidth=1.5,
                        label=f"±0.1·median bounds",
                    )
                    plt.axvline(
                        upper_small, color="orange", linestyle="--", linewidth=1.5
                    )
                    plt.axvline(
                        lower_5s,
                        color="magenta",
                    ...
                    plt.axvline(
                        ...
                        linestyle=":",
                        linewidth=2,
                        label=f"+5σ = {upper_5s:.2f}",
                    )
                    plt.title(
                        "Histogram used for sigma estimation (±0.1·median masked copy)"
                    )
                    plt.xlabel("ADU")
                    plt.ylabel("Counts")
                    ...
                    margin = (
                        span * 0.05 if span > 0 else max(abs(median_val) * 0.1, 1.0)
                    )
                ...
                plt.title(
                    f"Bad pixel mask (median ± 3σ) for bias config: {configuration}"
                )
                plt.colorbar()
                plt.show()
                ...
                plt.title(
                    f"Masked combined master bias (median ± 5σ) for config: {configuration}"
                )
                plt.colorbar()
                plt.show()
                ...
                try:
                    ...
                    fits.PrimaryHDU(data.astype(np.float32)).writeto(
                ...
                try:
                    if bad_pixel_mask is None:
                        self.logger.warning(
                            f"No bad pixel mask for bias config {key}; skipping write"
                        )
                    else:
                        bpm_dir = os.path.join(
                            self.raw_data_path, "master_biases", "bad_pixel_masks"
                        )
                        os.makedirs(bpm_dir, exist_ok=True)
                        outpath_bpm = os.path.join(
                            bpm_dir, f"bad_pixel_mask_bias_{key:03d}.fits"
                        )
                        ...
                        if (
                            hasattr(combined_median, "header")
                            and combined_median.header is not None
                        ):
                            if isinstance(combined_median.header, fits.Header):
                                ...
                                try:
                                    ...
                                    self.logger.warning(
                                        f"Could not add header key {hkey}={hvalue}: {header_error}"
                                    )
                        else:
                            # Create new header if none exists
                            header = fits.Header()

                        fits.PrimaryHDU(mask_to_write, header=header).writeto(
                            outpath_bpm, overwrite=True
                        )
                        self.logger.info(
                            f"Wrote bad pixel mask for bias config {key} to {outpath_bpm}"
                        )
                except Exception as e:
                    self.logger.error(
                        f"Failed to write bad pixel mask for bias config {key} to disk: {e}"
                    )
    ...
    def load_master_biases(self):
        ...
        try:
            ...
            if not os.path.isdir(master_bias_dir):
                self.logger.warning(
                    f"Master bias directory {master_bias_dir} does not exist"
                )
                return
            ...
            for filename in os.listdir(master_bias_dir):
                ...
                if hdul is None:
                    self.logger.warning(
                        f"Could not open master bias file {filepath}, skipping"
                    )
                    continue
                data = hdul[0].data
                ...
                try:
                    ...
                    key_str = str(key_int)
                    self.master_biases[key_str] = CCDData(
                        data, unit=u.adu, header=header
                    )
                    self.logger.info(
                        f"Loaded master bias from {filepath} with key {key_str}"
                    )
                except ValueError as e:
                    self.logger.warning(
                        f"Could not parse key from filename {filename}: {e}"
                    )
                    continue
        except Exception as e:
    ...
    def load_bad_pixel_masks_bias(self):
        ...
        try:
            ...
            if not os.path.isdir(bpm_dir):
                self.logger.warning(
                    f"Bad pixel mask directory {bpm_dir} does not exist"
                )
                return
            ...
            for filename in os.listdir(bpm_dir):
                ...
                if hdul is None:
                    self.logger.warning(
                        f"Could not open bad pixel mask file {filepath}, skipping"
                    )
                    continue
                ...
                try:
                    ...
                    self.logger.info(
                        f"Loaded bad pixel mask from {filepath} with key {key_str}"
                    )
                except ValueError as e:
                    self.logger.warning(
                        f"Could not parse key from filename {filename}: {e}"
    ...
    def load_bad_pixel_masks_flats(self):
        ...
        try:
            ...
            for filename in os.listdir(bpm_dir):
                ...
                header = hdul[0].header
                # Extract the key properly - remove prefix and suffix, then convert to int then to str
                key_part = filename.replace("bad_pixel_mask_bias_", "").replace(
                    ".fits", ""
                )
                try:
                    # Convert to int first to handle zero-padding, then back to str for consistency with storage
    ...
    def determine_flat_configurations(self):
        ...
        if not getattr(self, "setup_table", None):
            self.logger.info(
                "No setup table available to determine flat configurations."
            )
            self.flat_configurations = {}
            self.science_to_flat_map = {}
        ...
        next_idx = (
            max(self.flat_configurations.keys()) + 1 if self.flat_configurations else 0
        )
        ...
        for ff in flat_files:
            ...
            try:
                ...
                self.logger.warning(
                    f"Failed to read window keyword using get_header_value for flat file {ff}: {e}"
                )
                w = "UNKNOWN"
            try:
                bx = (
                ...
                self.logger.warning(
                    f"Failed to read bin_x keyword using get_header_value for flat file {ff}: {e}"
                )
                bx = "UNKNOWN"
            try:
                by = (
                ...
                self.logger.warning(
                    f"Failed to read bin_y keyword using get_header_value for flat file {ff}: {e}"
                )
                by = "UNKNOWN"
            ...
            try:
                ...
                self.logger.warning(
                    f"Failed to read filter keywords using get_header_values for flat file {ff}: {e}"
                )
            ...
            if matched_idx is None:
                ...
                next_idx += 1
                self.logger.info(
                    f"Created new flat configuration {matched_idx} for key {key} and added file {ff}"
                )
            else:
                files_list = self.flat_configurations[matched_idx].setdefault(
                    "files", []
                )
                if ff not in files_list:
                    files_list.append(ff)
                    self.logger.info(
                        f"Appended flat file {ff} to flat configuration {matched_idx}"
                    )
        ...
        self.logger.info(
            f"Found {len(self.flat_configurations)} unique flat configurations"
        )
        ...
        try:
            ...
            serializable_map = {
                str(k): (v if v is not None else None)
                for k, v in self.science_to_flat_map.items()
            }
            with open(map_outpath, "w", encoding="utf-8") as fh:
                json.dump(serializable_map, fh, indent=2, ensure_ascii=False)
    ...
    def make_master_flats(self):
        ...
        for key, configuration in self.flat_configurations.items():
            ...
            for flat_file in configuration.get("files", []):
                ...
                if data is None:
                    self.logger.warning(
                        f"Could not read flat file {flat_file}, skipping"
                    )
                    continue
            ...
            if master_flat is not None:
                ...
                if self.instrument.name != "NOTCAM":
                    ...
                    bad_pixel_mask_bias = self.bad_pixel_masks_bias.get(
                        str(bias_conf_idx)
                    )

                    if bad_pixel_mask_bias is None:
                        self.logger.error(
                            f"No bad pixel mask found for bias config idx {bias_conf_idx} used in flat config {key}"
                        )
                        exit(-1)

                    masked_bias = np.ma.masked_array(
                        bias_frame.data, mask=bad_pixel_mask
                    )
                    ...
                    plt.title(
                        f"Bias frame for bias config idx {bias_conf_idx} used in flat config {key}"
                    )
                    plt.colorbar()
                    plt.show()
                    ...
                    masked_flat = np.ma.masked_array(
                        combined_median.data, mask=bad_pixel_mask
                    )
                ...
                non_finite = ~finite_mask
                mask_small = (
                    non_finite | (work_arr < lower_small) | (work_arr > upper_small)
                )
                ...
                mean = (
                    float(np.mean(masked_data_5s))
                    if masked_data_5s.size > 0
                    else median_val
                )

                normalized_flat = combined_median.data / mean

                normalized_flat_masked = np.ma.masked_array(
                    normalized_flat, mask=bad_pixel_mask_flat
                )

                try:
                    size_plot = min(kept_vals_small.size, 200_000)
                    idxs = np.random.choice(
                        kept_vals_small.size, size=size_plot, replace=False
                    )
                ...
                try:
                    ...
                    plt.title(
                        "Histogram used for sigma estimation (±0.2·median masked copy)"
                    )
                    plt.xlabel("ADU")
                    plt.ylabel("Counts")
                    plt.legend(loc="upper right")
                ...
                try:
                    ...
                    plt.title(
                        f"Masked combined master flat (median ± 5σ) for config idx: {key}"
                    )
                    plt.colorbar()
                    plt.show()
                except Exception:
                ...
                master_flat = CCDData(
                    normalized_flat, unit=u.adu, header=combined_median.header
                )
                ...
                try:
                    ...
                    fits.PrimaryHDU(master_flat.data.astype(np.float32)).writeto(
                        outpath, overwrite=True
                    )
                ...
                try:
                    if bad_pixel_mask_flat is None:
                        self.logger.warning(
                            f"No bad pixel mask to write for flat config {key}; skipping BPM write"
                        )
                    else:
                        bpm_dir = os.path.join(
                            self.raw_data_path, "master_flats", "bad_pixel_masks"
                        )
                        os.makedirs(bpm_dir, exist_ok=True)
                        ...
                        self.logger.info(
                            f"Wrote bad pixel mask for flat config {key} to {outpath_bpm}"
                        )
                except Exception as e:
                    self.logger.error(
                        f"Failed to write bad pixel mask for flat config {key} to disk: {e}"
                    )
    ...
    def load_master_flats(self):
        ...
        try:
            ...
            if not os.path.isdir(master_flat_dir):
                self.logger.warning(
                    f"Master flat directory {master_flat_dir} does not exist"
                )
                return
            ...
            for filename in os.listdir(master_flat_dir):
                ...
                if hdul is None:
                    self.logger.warning(
                        f"Could not open master flat file {filepath}, skipping"
                    )
                    continue
                data = hdul[0].data
                header = hdul[0].header
                ...
                try:
                    ...
                    key_int = int(key_part)
                    key_str = str(key_int)
                    self.master_flats[key_str] = CCDData(
                        data, unit=u.adu, header=header
                    )
                    self.logger.info(
                        f"Loaded master flat from {filepath} with key {key_str}"
                    )
                except ValueError as e:
                    self.logger.warning(
                        f"Could not parse key from filename {filename}: {e}"
    ...
    def load_bad_pixel_masks_flats(self):
        ...
        try:
            ...
            for filename in os.listdir(bpm_dir):
                ...
                header = hdul[0].header
                # Extract the key properly - remove prefix and suffix, then convert to int then to str
                key_part = filename.replace("bad_pixel_mask_flat_", "").replace(
                    ".fits", ""
                )
    ...
    def reduce(self):
        ...
        for key, configuration in self.setup_table.items():
            print(
                f"Reducing science frame with setup config idx {key}: {configuration}"
            )
            ...
            bias_frame = (
                self.master_biases.get(str(bias_idx)) if bias_idx is not None else None
            )
            ...
            for fname in configuration.get("files", []):
                ...
                if hdul_tmp is None:
                    self.logger.warning(
                        f"Could not open {fname} to read OBJECT header, skipping for object mapping"
                    )
                    continue
                try:
                    obj = (
                        self.instrument.get_header_value(
                            hdul_tmp, self.instrument.object_keyword
                        )
                        or "UNKNOWN"
                    )
                except Exception as e:
                    self.logger.warning(
                        f"Failed to read object name from header for {fname}: {e}"
                    )
                    obj = "UNKNOWN"
                key = str(obj)
            ...
            # optional quick log
            self.logger.info(
                f"Updated object->files map; total objects tracked: {len(self.object_to_files)}"
            )

            for file in configuration.get("files", []):

                self.logger.info(
                    f"Reducing science file {file} with setup config idx {key}"
                )
                ...
                try:
                    obj_name = (
                        self.instrument.get_header_value(
                            hdul, self.instrument.object_keyword
                        )
                        or "UNKNOWN"
                    )
                    ...
                    self.logger.warning(
                        f"Failed to read object name from header for file {file}: {e}"
                    )

                if bias_frame is None and self.instrument.name != "NOTCAM":
                    self.logger.error(
                        f"No master bias found for science config idx {key}, skipping reduction for file {file}"
                    )
                    continue

                if flat_frame is None:
                    self.logger.error(
                        f"No master flat found for science config idx {key}, skipping reduction for file {file}"
                    )
                    continue

                if bad_pixel_mask is None:
                    self.logger.error(
                        f"No bad pixel mask found for science config idx {key}, skipping reduction for file {file}"
                    )
                    continue
                ...
                science_arr = (
                    science_data.data
                    if hasattr(science_data, "data")
                    else np.asarray(science_data)
                )
                masked_science = np.ma.masked_array(science_arr, mask=bad_pixel_mask)

                has_bias = (bias_frame is not None) and (
                    self.instrument.name != "NOTCAM"
                )
                masked_bias = (
                    np.ma.masked_array(bias_frame.data, mask=bad_pixel_mask)
                    if has_bias
                    else None
                )
                ...
                sci_vals = (
                    science_arr[~bad_pixel_mask]
                    if np.any(~bad_pixel_mask)
                    else science_arr.ravel()
                )
                smin, smax = safe_percentiles(sci_vals)

                flat_vals = (
                    masked_flat.compressed()
                    if hasattr(masked_flat, "compressed")
                    else np.asarray(masked_flat).ravel()
                )
                fmin, fmax = safe_percentiles(flat_vals)

                red_vals = (
                    reduced_masked.compressed()
                    if hasattr(reduced_masked, "compressed")
                    else np.asarray(reduced_masked).ravel()
                )
                rmin, rmax = safe_percentiles(red_vals)

                if has_bias:
                    bias_vals = (
                        masked_bias.compressed()
                        if hasattr(masked_bias, "compressed")
                        else np.asarray(masked_bias).ravel()
                    )
                ...
                if True:
                    ...
                    im0 = ax_list[0].imshow(
                        science_arr, origin="lower", cmap="gray", vmin=smin, vmax=smax
                    )
                ...
                try:
                    filter_key = (
                        ",".join([str(f) for f in filter_tuple])
                        if filter_tuple
                        else "UNKNOWN"
                    )
                except Exception:
                ...
                try:
                    arr = (
                        reduced_masked.filled(np.nan)
                        if hasattr(reduced_masked, "filled")
                        else np.asarray(reduced_masked)
                    )
                ...
                if (
                    obj_key not in self.reduced_objects
                    or self.reduced_objects.get(obj_key) is None
                ):
                ...
                if existing is None:
                    ...
                    if (
                        ...
                        and existing.ndim == 3
                        and existing.shape[1:] == arr.shape
                    ):
                        try:
                            obj_dict[filter_key] = np.concatenate(
                                [existing, arr[np.newaxis, ...]], axis=0
                            )
                        except Exception as e:
                            self.logger.warning(
                                f"Failed to append reduced frame for object {obj_key}, filter {filter_key}: {e}"
                            )
                    else:
                        self.logger.warning(
                ...
                for filter_key, cube in (obj_dict or {}).items():
                    try:
                        if not isinstance(cube, np.ndarray):
                            self.logger.warning(
                                f"Skipping non-array datacube for object={obj_key} filter={filter_key}"
                            )
                            continue
                        if cube.ndim != 3:
                            self.logger.warning(
                                f"Skipping datacube with invalid ndim for object={obj_key} filter={filter_key}: ndim={cube.ndim}"
                            )
                            continue
                        ...
                        if n_frames < 3:
                            self.logger.info(
                                f"Not enough frames to compute median for object={obj_key} filter={filter_key}: found {n_frames} (need >= 3), skipping"
                            )
                            continue
                        ...
                        plt.title(
                            f"Sum of {n_frames} frames for object={obj_key} filter={filter_key}"
                        )
                        plt.show()
                        ...
                        combiner.sigma_clipping(
                            low_thresh=0.5, high_thresh=0.5, func=np.ma.median
                        )
                        ...
                        try:
                            ...
                            if n_panels > max_panels:
                                self.logger.info(
                                    f"Too many frames ({n_frames}) to display; showing first {max_panels-1} frames plus median"
                                )
                            ...
                            if finite_vals.size == 0:
                                ...
                                if (
                                    ...
                                    vmax = (
                                        ...
                                        if np.isfinite(np.nanmax(finite_vals))
                                        else vmin + 1.0
                                    )
                            ...
                            try:
                                fig.colorbar(
                                    im, ax=axs_flat.tolist(), fraction=0.02, pad=0.01
                                )
                            except Exception:
                                pass
                            ...
                            self.logger.warning(
                                f"Failed to plot median + frames for object={obj_key} filter={filter_key}: {e}"
                            )

                        # subtract the median from every frame in the datacube and display the residuals
                        diffs = (
                            cube - median_img[np.newaxis, ...]
                        )  # broadcast median to cube shape
                        ...
                        for i in range(diffs.shape[0]):
                            ...
                            try:
                                ...
                                if not np.isfinite(vmax) or vmax == 0:
                                    vmax = (
                                        float(np.nanmax(np.abs(frame)))
                                        if np.isfinite(np.nanmax(np.abs(frame)))
                                        else 1.0
                                    )
                            except Exception:
                                vmax = 1.0
                            ...
                            try:
                                ...
                                if (
                                    not np.isfinite(mmin)
                                    or not np.isfinite(mmax)
                                    or mmin == mmax
                                ):
                                    mmin = (
                                        float(np.nanmin(median_img))
                                        if np.isfinite(np.nanmin(median_img))
                                        else 0.0
                                    )
                                    mmax = (
                                        float(np.nanmax(median_img))
                                        if np.isfinite(np.nanmax(median_img))
                                        else mmin + 1.0
                                    )
                            except Exception:
                                mmin = (
                            ...
                            try:
                                ...
                                if (
                                    not np.isfinite(omin)
                                    or not np.isfinite(omax)
                                    or omin == omax
                                ):
                                    omin = (
                                        float(np.nanmin(orig))
                                        if np.isfinite(np.nanmin(orig))
                                        else 0.0
                                    )
                                    omax = (
                                        float(np.nanmax(orig))
                                        if np.isfinite(np.nanmax(orig))
                                        else omin + 1.0
                                    )
                            except Exception:
                                omin = (
                            ...
                            im0 = axs[0].imshow(
                                orig, origin="lower", cmap="gray", vmin=omin, vmax=omax
                            )
                            axs[0].set_title(
                                f"Original frame (Object={obj_key} Filter={filter_key})"
                            )
                            axs[0].axis("off")
                            cbar0 = fig.colorbar(
                                im0, ax=axs[0], fraction=0.046, pad=0.04
                            )
                            ...
                            axs[1].axis("off")
                            cbar1 = fig.colorbar(
                                im1, ax=axs[1], fraction=0.046, pad=0.04
                            )
                            ...
                            axs[2].axis("off")
                            cbar2 = fig.colorbar(
                                im2, ax=axs[2], fraction=0.046, pad=0.04
                            )
                            ...
                            try:
                                ...
                                plt.ylabel("ADU")
                                plt.title(
                                    f"Central column slice (col={mid_col}) Object={obj_key} Filter={filter_key} Frame={i}"
                                )
                                plt.legend(loc="best")
                                ...
                                self.logger.warning(
                                    f"Could not plot central column slice for object={obj_key} filter={filter_key} frame={i}: {e}"
                                )

                    except Exception as e:
                        self.logger.error(
                            f"Failed to display median for object={obj_key} filter={filter_key}: {e}"
                        )
        except Exception as e:
            self.logger.error(
                f"Error while iterating reduced objects to show medians: {e}"
            )
